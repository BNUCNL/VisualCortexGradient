...
def calc_RSM1(Hemi, mask, out_file):
    """
    计算各map之间的对称相关矩阵
    """
    ...
    map_names.extend(['distFromCalc+MT', 'distFromCalc+MT=V4', 'distFromOP+MT',
                      'distFromOP+MT=V4', 'distFromEDLV'])
    maps.extend([map_dist_model1, map_dist_model2, map_dist_model3,
                 map_dist_model4, map_dist_model5])
    ...
    # RSFC矩阵的PCA成分
    fpath = pjoin(anal_dir, 'decomposition/'
                  f'S1200-grp-RSFC-z_MMP-vis3-{Hemi}2MMP-vis3-{Hemi}_zscore_PCA-comp.dscalar.nii')
    reader = CiftiReader(fpath)
    map_rsfc = reader.get_data()[:, :LR_count_32k][:, mask]
    map_names.extend(f'RSFC-{i}' for i in reader.map_names())
    maps.append(map_rsfc)

    # 单结构模态PCA成分
    # Myelin
    fpath = pjoin(anal_dir, 'decomposition/'
                  f'HCPY-M_MMP-vis3-{Hemi}_zscore1_PCA-subj.dscalar.nii')
    reader = CiftiReader(fpath)
    map_M_pcs = reader.get_data()[:, mask]
    map_names.extend(f'M-{i}' for i in reader.map_names())
    maps.append(map_M_pcs)
    
    # Thickness
    fpath = pjoin(anal_dir, 'decomposition/'
                  f'HCPY-corrT_MMP-vis3-{Hemi}_zscore1_PCA-subj.dscalar.nii')
    reader = CiftiReader(fpath)
    map_corrT_pcs = reader.get_data()[:, mask]
    map_names.extend(f'corrT-{i}' for i in reader.map_names())
    maps.append(map_corrT_pcs)
...
def calc_RSM1_main(mask_name):
    ...
    if mask_name in ('MMP-vis3-L', 'MMP-vis3-R'):
        Hemi = mask_name[-1]
        mask = Atlas('HCP-MMP').get_mask(get_rois(mask_name))[0]
        ...
        calc_RSM1(
            Hemi=Hemi, mask=np.logical_and(R2_mask, mask),
            out_file=pjoin(work_dir, f'RSM1_{mask_name}_R2.pkl')
        )

    elif mask_name in ('Wang2015-L', 'Wang2015-R'):
        Hemi = mask_name[-1]
        mask1 = Atlas('HCP-MMP').get_mask(get_rois(f'MMP-vis3-{Hemi}'))[0]
        # R_FEF和R_IPS5与MMP-vis3-R没有重合的部分，前者在额叶，后者本身只有3个顶点。
        mask2 = Atlas('Wang2015').get_mask(get_rois(mask_name))[0]
    ...
    elif mask_name in ('EDLV-watershed-L', 'EDLV-watershed-R'):
        Hemi = mask_name[-1]
        rois = ('early', 'dorsal', 'lateral', 'ventral')
        labels = [f'{Hemi}_{i}' for i in rois]
        ...
        for lbl_idx, lbl in enumerate(labels):
            roi = rois[lbl_idx]
            fname = f'RSM1_{mask_name}-{roi}'
            mask = edlv_map == lbl2key[lbl]
            calc_RSM1(
                Hemi=Hemi, mask=mask,
                out_file=pjoin(work_dir, f'{fname}.pkl')
            )
            calc_RSM1(
                Hemi=Hemi, mask=np.logical_and(R2_mask, mask),
                out_file=pjoin(work_dir, f'{fname}_R2.pkl')
            )

    elif mask_name in ('EDLV-MMP-vis3-L', 'EDLV-MMP-vis3-R'):
        Hemi = mask_name[-1]
        rois = ('early', 'dorsal', 'lateral', 'ventral')
        labels = [f'{Hemi}_{i}' for i in rois]
...
def calc_RSM11(meas_name, Hemi):
    """
    计算PC1和PC2与某种测量在各mask内部的相关
    """
    atlas_mmp = Atlas('HCP-MMP')
    vis_name = f'MMP-vis3-{Hemi}'
    edlv_names = ['early', 'dorsal', 'lateral', 'ventral']
    vis_mask = atlas_mmp.get_mask(get_rois(vis_name))[0]
    R2_mask = nib.load(s1200_avg_R2).get_fdata()[0, :LR_count_32k] > 9.8
    out_file = pjoin(work_dir, f'RSM11_{meas_name}_{Hemi}.pkl')
    ...
    if meas_name == 'Eccentricity':
        meas_map = nib.load(s1200_avg_eccentricity).get_fdata()[0, :LR_count_32k]
    elif meas_name == 'Angle':
        meas_map = nib.load(s1200_avg_angle).get_fdata()[0, :LR_count_32k]
    elif meas_name == 'RFsize':
        meas_map = nib.load(s1200_avg_RFsize).get_fdata()[0, :LR_count_32k]
    elif meas_name == 'AngleFlip':
        meas_map = nib.load(pjoin(
            proj_dir, 'data/'
            'S1200_7T_Retinotopy181.Fit1_PolarAngleFlip_MSMAll.32k_fs_LR.dscalar.nii'
        )).get_fdata()[0, :LR_count_32k]
    else:
        raise ValueError
    non_nan_mask = ~np.isnan(meas_map)

    # prepare mask
    mask_names = []
    mask_maps = []

    mask_names.append(vis_name)
    mask_maps.append(vis_mask)
    mask_names.append(f'{vis_name}_R2')
    mask_maps.append(np.logical_and(vis_mask, R2_mask))

    atlas_mmp_edlv = Atlas('MMP-vis3-EDLV')
    for edlv_n in edlv_names:
        mask_names.append(f'{vis_name}_{edlv_n}')
        mask_maps.append(atlas_mmp_edlv.get_mask(f'{Hemi}_{edlv_n}')[0])
        if edlv_n == 'early':
            mask_names.append(f'{vis_name}_{edlv_n}_R2')
            mask_maps.append(np.logical_and(
                mask_maps[-1], R2_mask))

    atlas_wang = Atlas('Wang2015')
    mask_names.append(f'Wang2015-{Hemi}')
    mask_maps.append(np.logical_and(
        vis_mask, atlas_wang.get_mask(get_rois(mask_names[-1]))[0]
    ))

    wang_edlv_file = pjoin(proj_dir, 'data/wang2015/wang2015_4region.32k_fs_LR.dlabel.nii')
    wang_edlv_reader = CiftiReader(wang_edlv_file)
    wang_edlv_map = wang_edlv_reader.get_data()[0]
    wang_edlv_lbl_tab = wang_edlv_reader.label_tables()[0]
    wang_edlv_name2key = {}
    for edlv_k in np.unique(wang_edlv_map):
        edlv_n = wang_edlv_lbl_tab[edlv_k].label
        wang_edlv_name2key[edlv_n] = edlv_k
    for edlv_n in edlv_names:
        mask_names.append(f'Wang2015-{Hemi}_{edlv_n}')
        wang_edlv_mask = wang_edlv_map == wang_edlv_name2key[f'{Hemi}_{edlv_n}']
        mask_maps.append(np.logical_and(vis_mask, wang_edlv_mask))

    # calc pearsonr
    n_pc = len(pc_names)
    n_mask = len(mask_names)
    rs = np.zeros((n_pc, n_mask))
    ps = np.zeros((n_pc, n_mask))
    for mask_idx in range(n_mask):
        mask = np.logical_and(mask_maps[mask_idx], non_nan_mask)
        for pc_idx in range(n_pc):
            r, p = pearsonr(pc_maps[pc_idx, mask], meas_map[mask])
            rs[pc_idx, mask_idx] = r
            ps[pc_idx, mask_idx] = p

    # save
    data = {'row_name': pc_names, 'col_name': mask_names, 'r': rs, 'p': ps}
    pkl.dump(data, open(out_file, 'wb'))


if __name__ == '__main__':
    # calc_RSM1_main(mask_name='MMP-vis3-L')
    calc_RSM1_main(mask_name='MMP-vis3-R')
    # calc_RSM1_main(mask_name='Wang2015-L')
    # calc_RSM1_main(mask_name='Wang2015-R')
    # calc_RSM1_main(mask_name='EDLV-watershed-L')
    # calc_RSM1_main(mask_name='EDLV-watershed-R')
    # calc_RSM1_main(mask_name='EDLV-MMP-vis3-L')
    # calc_RSM1_main(mask_name='EDLV-MMP-vis3-R')
    ...
    # calc_RSM11(meas_name='Eccentricity', Hemi='L')
    # calc_RSM11(meas_name='Angle', Hemi='L')
    # calc_RSM11(meas_name='RFsize', Hemi='L')
    # calc_RSM11(meas_name='AngleFlip', Hemi='L')
    # calc_RSM11(meas_name='Eccentricity', Hemi='R')
    # calc_RSM11(meas_name='Angle', Hemi='R')
    # calc_RSM11(meas_name='RFsize', Hemi='R')
    # calc_RSM11(meas_name='AngleFlip', Hemi='R')
