...
from cxy_visual_dev.lib.predefine import proj_dir, mmp_name2label,\
    ...
    s1200_group_rsfc_mat, LR_count_32k, Hemi2stru, s1200_avg_angle
...
def make_EDLV_dlabel1():
    """
    依据HCP MMP的22组，将以下五组分别标记为四个通路
    Early: Group1+2
    Dorsal: Group3
    Lateral: Group5
    Ventral: Group4
    ...
    reader = CiftiReader(mmp_map_file)
    atlas = Atlas('HCP-MMP')
    out_file = pjoin(proj_dir, 'data/HCP/HCP-MMP1_EDLV.dlabel.nii')
    ...
    # dorsal
    rois_dorsal = get_rois('MMP-vis3-G3')
    ...
    # ventral
    rois_ventral = get_rois('MMP-vis3-G4')
...
def split_subj_half():
    ...
    将被试随机分半
    """
    sid_file = pjoin(proj_dir, 'data/HCP/HCPY_SubjInfo.csv')
    out_file = pjoin(proj_dir, 'data/HCP/HCPY_SubjInfo_halfID.csv')

    df = pd.read_csv(sid_file)
    n_sid = df.shape[0]
    sidx_vec = np.arange(n_sid)

    out_dict = {'subID': df['subID']}
    sidx_vec_half = np.random.choice(sidx_vec, int(n_sid/2), replace=False)
    half_id_vec = np.zeros(n_sid, dtype=np.uint8)
    half_id_vec[sidx_vec_half] = 1
    out_dict['halfID'] = half_id_vec

    df = pd.DataFrame(out_dict)
    df.to_csv(out_file, index=False)


def merge_gii2cii_margulies():
    Hemis = ('L', 'R')
    work_dir = pjoin(proj_dir, 'data/neuromaps-data/annotations/margulies2016')
    grad_names = sorted(os.listdir(work_dir))
    src_files = pjoin(work_dir, '{grad_name}/fsLR/'
                      'source-margulies2016_desc-{grad_name}_space-fsLR_den-32k_hemi-{Hemi}_feature.func.gii')
    out_file = pjoin(work_dir,
                     'source-margulies2016_space-fsLR_den-32k_feature.dscalar.nii')

    reader = CiftiReader(mmp_map_file)
    data = np.zeros((len(grad_names), LR_count_32k))
    for Hemi in Hemis:
        offset, count, map_shape, index2v = reader.get_stru_pos(Hemi2stru[Hemi])
        for grad_idx, grad_name in enumerate(grad_names):
            src_file = src_files.format(grad_name=grad_name, Hemi=Hemi)
            grad_map = nib.load(src_file).darrays[0].data
            data[grad_idx, offset:(offset+count)] = grad_map[index2v]
    save2cifti(out_file, data, reader.brain_models(), grad_names)


def unify_up_low_FOV():
    """
    Unify the values of the upper and lower field of view
    我们只要以x=180为对称轴，把第三和四象限翻转过去就好了。
    公式为：180-(x-180)。
    理解为：d=x-180是x离180的距离（带符号），于是x=180+d。那么对称的值就应该是180-d
    """
    reader = CiftiReader(s1200_avg_angle)
    data = reader.get_data()
    low_FOV_mask = data > 180
    data[low_FOV_mask] = 360 - data[low_FOV_mask]
    save2cifti(pjoin(
        proj_dir,
        'data/S1200_7T_Retinotopy181.Fit1_PolarAngleFlip_MSMAll.32k_fs_LR.dscalar.nii'
    ), data, reader.brain_models(), reader.map_names(), reader.volume)
