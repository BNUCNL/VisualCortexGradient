...
def weight_CCA_beh(vis_name):
    ...
    beh_cols = [
        ...
        'Mars_Final']
    # beh_cols = [
    #     'DDisc_AUC_200', 'DDisc_AUC_40K', 'Noise_Comp',
    #     'PainIntens_RawScore', 'PainInterf_Tscore', 'Taste_Unadj']
    ...
    out_file = pjoin(work_dir, f'weight-CCA-beh_{vis_name}_v31.pkl')
...
def stru_PC_fit_beh_PC():
    ...
    stru_names = ('M', 'T')
    postfix = 'v1_IQR2'
    data_file = pjoin(anal_dir, 'decomposition/'
                      f'PCA_beh_data_{postfix}.pkl')
    out_file = pjoin(work_dir, f'stru_PC_fit_beh_PC_{postfix}.pkl')

    # loading
    data = pkl.load(open(data_file, 'rb'))
    out_dict = {'beh_pc': data['PCA']['component_name'],
                'arr_shape': '(n_beh_pc,)'}
    n_beh_pc = len(out_dict['beh_pc'])
    for Hemi in Hemis:
        for stru_pc in stru_pcs:
            k = f'{Hemi}_{stru_pc}'
            out_dict[k] = {}
            out_dict[k]['r'] = np.zeros(n_beh_pc)
            out_dict[k]['p'] = np.zeros(n_beh_pc)
            print(f'==={k}===')
            # prepare X, Y
            feat_names = [f'{k}_{i}_weight'
                          for i in stru_names]
            print(feat_names)
            feat_indices = [data['weight_name'].index(i)
                            for i in feat_names]
            X = data['weight_arr'][:, feat_indices]
            Y = data['PCA']['pc_arr'].T
            ...
            Y_pred = model.predict(X)
            for beh_pc_idx in range(n_beh_pc):
                r, p = pearsonr(Y[:, beh_pc_idx], Y_pred[:, beh_pc_idx],
                                alternative='greater')
                out_dict[k]['r'][beh_pc_idx] = r
                out_dict[k]['p'][beh_pc_idx] = p
    pkl.dump(out_dict, open(out_file, 'wb'))
